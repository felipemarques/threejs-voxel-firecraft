...
/**
 * Creates a fat male player character mesh with prominent belly
 * Based on MalePlayerObject with added belly sphere like FatZombie
 */
export function createFatMalePlayer(options: FatPlayerOptions = {}) {
 ...
 const material = new THREE.MeshStandardMaterial({ color: shirtColor }) // Shirt
  ...
  if (showGlasses) {
    ...
    // Arms
    const glassLeftArm = new THREE.Mesh(
    ...
    const glassRightArm = new THREE.Mesh(
      new THREE.BoxGeometry(0.02, 0.02, 0.3),
      glassesMat
    )
...
function buildMouthMesh(style: MouthStyle): THREE.Object3D | null {
...
function createWeapon(type: WeaponType): THREE.Group | null {
