 ...
 * Based on v1/src/objects/motorcycleElement.js
 */
export function createMotorcycle() {
  ...
  // Main frame
  const frame = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.3, 1.8), bodyMat)
  frame.position.y = 0.8
  frame.castShadow = true
  motoGroup.add(frame)

  // Engine block
  const engine = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.5, 0.7), chromeMat)
  engine.position.set(0, 0.6, 0.2)
  engine.castShadow = true
  motoGroup.add(engine)

  // Front wheel
  const frontWheel = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.5, 0.2, 16), wheelMat)
  ...
  // Rear wheel
  const rearWheel = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.6, 0.25, 16), wheelMat)
  ...
  // Fork
  const fork = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.8, 0.15), chromeMat)
  ...
  // Handlebars
  const handlebar = new THREE.Mesh(new THREE.BoxGeometry(1, 0.1, 0.1), chromeMat)
  handlebar.position.set(0, 1.3, 1.1)
  motoGroup.add(handlebar)

  // Handlebar grips
  ;[-0.45, 0.45].forEach((x) => {
    const grip = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.2, 8), bodyMat)
  ...
  // Seat
  const seat = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.15, 0.8), seatMat)
  seat.position.set(0, 1.1, -0.2)
  seat.castShadow = true
  motoGroup.add(seat)

  // Fuel tank
  const tank = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.4, 0.6), bodyMat)
  ...
  const headlight = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xffffcc, emissive: 0xffffcc, emissiveIntensity: 0.3 })
  )
  ...
  // Exhaust pipe
  const exhaust = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.1, 0.8, 8), chromeMat)
